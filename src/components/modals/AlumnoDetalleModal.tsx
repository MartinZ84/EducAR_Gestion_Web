...
import TutorAsignacionModal from './TutorAsignacionModal';
...
export default function AlumnoDetalleModal({ open, id, onClose }: { open: boolean; id: number | null; onClose: () => void }) {
  ...
  const [gestionarTutores, setGestionarTutores] = useState(false);
  const [reloadKey, setReloadKey] = useState(0);
  return <><DetalleModal open={open} id={id} title="Detalle del alumno" load={getAlumnoDetalle} onClose={onClose} reloadKey={reloadKey}>
    {(data: AlumnoDetalle) => <Stack spacing={2}>
      ...
      {tab === 2 && <Stack spacing={2}><Button variant="outlined" sx={{ alignSelf: 'flex-start' }} onClick={() => setGestionarTutores(true)}>Gestionar tutores</Button><TablaVacia data={data.tutores} empty="No tiene tutores asignados." headers={['Tutor', 'Parentesco']} render={(item) => <TableRow key={item.idTutor}><TableCell>{item.nombreCompleto}</TableCell><TableCell>{item.parentesco || 'Sin especificar'}</TableCell></TableRow>} /></Stack>}
      {tab === 3 && <Stack spacing={2}><Stack direction="row" spacing={1} sx={{ flexWrap: 'wrap' }}><Chip label={`Presentes: ${data.asistenciaResumen.presentes}`} color="success" /><Chip label={`Ausentes: ${data.asistenciaResumen.ausentes}`} color="error" /><Chip label={`Justificadas: ${data.asistenciaResumen.justificadas}`} /></Stack><TablaVacia data={data.asistencias} empty="No hay asistencias registradas." headers={['Fecha', 'Estado']} render={(item) => <TableRow key={item.idAsistencia}><TableCell>{fecha(item.fecha)}</TableCell><TableCell>{item.estado}</TableCell></TableRow>} /></Stack>}
  ...
  </DetalleModal><TutorAsignacionModal open={gestionarTutores} idAlumno={id} obligatorio onClose={() => setGestionarTutores(false)} onChanged={() => setReloadKey(k => k + 1)} /></>;
}
